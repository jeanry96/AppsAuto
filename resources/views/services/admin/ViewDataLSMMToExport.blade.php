  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title"><strong> Data LSMM To <a href="{{ url('start-export') }}">Export</a></strong> (Total: <b>{{ $count }}</b> Items)</h2>
      </div>
    </div>
    <!-- div class="card">
      <div class="card-body">
      ...
      <div class="card-header">
        <table id="example2 table-datatables" class="table table-hover disply example2">
          ...
          <tr>
            ...
            <td>{{ $row->nominal }}</td>
            <td>{{ $row->signs }} </td>
            <td>{{ $row->ls }}-{!! \Illuminate\Support\Str::limit($row->nama_nasabah,7) !!}</td>
          </tr> 
          @endforeach
          </tbody>
          <tfoot>
