  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title"><strong> Data TPMI To <a href="{{ url('export/scls/telp') }}">Export</a></strong> (Total: <b>{{ $count }}</b> Items)</h2>
      </div>
    </div>
    <!-- div class="card">
      <div class="card-body">
      ...
      <div class="card-header">
        <table id="example2 table-datatables" class="table table-hover disply example2">
          ...
          <tr>
            ...
            <td>{{ $row->tagihan }}</td>
            <td>{{ $row->sign }} </td>
            <td>{{ $row->telpon }}/{!! \Illuminate\Support\Str::limit($row->nama_nasabah,5) !!}</td>
          </tr> 
          @endforeach
          </tbody>
          <tfoot>
